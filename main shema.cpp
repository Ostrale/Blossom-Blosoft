...
struct flow_info{
    ...
    uint16_t src_port;
    uint16_t dst_port;
    ndpi_protocol detected_protocol; //[x]
    bool is_tcp;
};
...
std::vector<const struct flow_struct *> archived_flows; // In Debug file
...
int read_packet(const flow_info &info_packet, const packet_struct &packet, const Tins::PDU &pdu) {
    flow_struct* flow_to_process;
    bool flow_found = false;
    for (auto &flow : flows) {
        if (compare_flow(*flow, info_packet)) {
            flow_found = true;
            flow_to_process = flow;
        }
    }
    if (!flow_found) {
        flow_struct *new_flow = new flow_struct;
        new_flow->info = info_packet;
        flows.push_back(new_flow);
        flow_to_process = new_flow;
    }
    flow_to_process->packets.push_back(packet);

    ndpi_finalize_initialization(ndpi_struct);

    flow_to_process->info.detected_protocol = ndpi_detection_process_packet(ndpi_struct, &flow_to_process->ndpi_flow, packet.packet_data, packet.packetlen, packet.timestamp, NULL);
    flow_heal_check(*flow_to_process, const_cast<Tins::PDU&>(pdu));
    return 0;
}

std::vector<App_Protocol> protocols;

void detected_protocols(const flow_struct &flow, std::vector<App_Protocol> &protocols) {
